...
import { translateBoatText, translateBoatSpec } from "@shared/boatTextTranslations";
...
export default function BoatDetailPage({ boatId = "solar-450", onBack }: BoatDetailPageProps) {
  ...
  // Name as the visitor reads it. Analytics keeps boatData.name so the metrics
  // don't fragment into eight labels per product.
  const boatName = translateBoatText(boatData?.name ?? "", language);
  ...
  const handleWhatsApp = () => {
    const message = t.boatDetail.whatsappPrefill.replace("{boatName}", boatName);
  ...
  const breadcrumbSchema = generateBreadcrumbSchema([
    ...
    { name: boatName, url: `/barco/${boatId}` },
  ]);
  ...
  const boatFaqTitle = buildBoatFaqTitle({ name: boatName }, boatFaqText);
  const boatFaqSchema = {
    "@type": "FAQPage",
          ...
          <div className="bg-background rounded-2xl overflow-hidden border border-card-border">
            ...
            >
              <img
                ...
                alt={getBoatAltText(boatName, currentImageIndex)}
                  ...
                  {displayImages.map((image, index) => (
                    ...
                    >
                      <img
                        ...
                        alt={getBoatAltText(boatName, index)}
                  ...
                  >
                    ...
                    >
                      <img
                        ...
                        alt={getBoatAltText(translateBoatText(relBoat.name, language))}
                    ...
                    <div className="p-4">
                      <h3 className="font-heading font-semibold text-foreground mb-1 truncate">
                        {translateBoatText(relBoat.name, language)}
                      </h3>
