...
export default function LocationMalgratPage() {
  ...
  useEffect(() => { trackLocationPageView("malgrat"); }, []);
  const t = useTranslations();
  const seoConfig = getSEOConfig('locationMalgrat', language);
  const hreflangLinks = generateHreflangLinks('locationMalgrat');
  const canonical = generateCanonicalUrl('locationMalgrat', language);

  const s = t.locationPages.malgrat.sections!;
  ...
  const breadcrumbSchema = generateBreadcrumbSchema([
    ...
    { name: t.breadcrumbs.locationMalgrat, url: "/alquiler-barcos-malgrat-de-mar" }
  ]);
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Hero Section */}
      <div className="bg-gradient-to-br from-amber-50 to-blue-50 pt-24 pb-12">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            <div className="flex items-center justify-center mb-6">
              <MapPin className="w-8 h-8 text-primary mr-4" />
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-heading font-bold text-foreground">
                {t.locationPages.malgrat.hero.title}
              </h1>
            </div>
            <p className="text-lg text-muted-foreground mb-6 max-w-4xl mx-auto">
              {t.locationPages.malgrat.hero.subtitle}
            </p>
            <div className="flex flex-wrap gap-3 justify-center">
              <Badge variant="outline" className="text-primary border-primary">
                <Car className="w-4 h-4 mr-2" />
                {t.locationPages.malgrat.hero.badgeDistance}
              </Badge>
              <Badge variant="outline" className="text-primary border-primary">
                <Clock className="w-4 h-4 mr-2" />
                {t.locationPages.malgrat.hero.badgeTime}
              </Badge>
              <Badge variant="outline" className="text-primary border-primary">
                <Waves className="w-4 h-4 mr-2" />
                {t.locationPages.malgrat.hero.badgeBeach}
              </Badge>
            </div>
          </div>
        </div>
          ...
          <Card className="bg-primary text-white">
            <CardContent className="py-8 text-center">
              ...
              <Button
                ...
                data-testid="button-whatsapp-malgrat"
              >
                {s.ctaButton}
              </Button>
            </CardContent>
      ...
      <div className="py-12 bg-background">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-heading font-bold text-center mb-8">
            Preguntas frecuentes sobre alquilar barco desde Malgrat de Mar
          </h2>
      ...
      <RelatedLocationsSection currentLocation="malgrat" />
