...
**Estado**: TIER 1 + TIER 2 + TIER 3 completos (7/7 ítems entregados el mismo día).
...
4. ✅ **Geo-IP defaults** — entregado 2026-05-21. Endpoint `GET /api/geo` (`server/routes/geo.ts`) usa `geoip-lite` (offline, sin API key, sin rate limit). Orden de detección: header `cf-ipcountry` → `x-vercel-ip-country` → IP del cliente vía `x-forwarded-for`/`req.ip` → `geoip-lite.lookup`. Devuelve `{ country: "FR" | null }`. Cache `max-age=300`. Excluye IPs locales/privadas en dev. Cliente en `LicenseVerifierPanel` cablea el upgrade respetando elección manual: `isAutoCountryRef` arranca a `true`, se voltea a `false` en `handleCountrySelect` y en hidratación desde sessionStorage. El fetch corre tras el language default; si el geo difiere y el usuario no ha tocado el picker, sustituye silenciosamente. Solo aplica si el iso2 del geo está en `LICENSE_COUNTRIES` (evita setear países que no se pueden seleccionar).
...
7. ✅ **Iconos PWA 192/512 maskable** — entregado 2026-05-21. Script `scripts/generate-maskable-icons.mjs` usa `sharp` para producir 3 nuevos PNG desde `logo-icon-512.png`: (a) `logo-icon-192.png` 192x192 any, downscale directo; (b) `logo-icon-192-maskable.png` 192x192 maskable, logo al 80% en canvas blanco; (c) `logo-icon-512-maskable.png` 512x512 maskable, mismo padding. La safe zone del 20% asegura que Android pueda recortar a circular/squircle sin perder el lockup "CB/RB". Manifest expone los 4 iconos PWA (192 any, 512 any, 192 maskable, 512 maskable) + el apple-touch-icon 180x180 legacy. Re-correr con `node scripts/generate-maskable-icons.mjs`.
