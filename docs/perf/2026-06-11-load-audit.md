...
## Service worker: enlaces vanity y `navigateFallbackDenylist` (2026-06-13)

Regla operativa para cualquier ruta que **el servidor** redirige (302/301) en vez de servir el SPA: hay que añadirla a `navigateFallbackDenylist` en `vite.config.ts`. Si no, el service worker trata la navegación como una ruta de SPA, sirve `/index.html` desde caché y muestra el 404 del cliente ("Página no encontrada") — la petición nunca llega al servidor, así que el redirect no se ejecuta.

Caso real: el enlace vanity `/resena` (y alias `/resenas`, `/review`, `/reviews`) que `server/index.ts` redirige 302 a Google Reviews (`GOOGLE_REVIEW_URL`, derivado de `BUSINESS_PLACE_ID` en `shared/businessProfile.ts`). El SW lo capturaba y mostraba el 404; solo funcionaba con hard refresh (que salta el SW). Fix en `69b3941`: patrón `/^\/(resena|resenas|review|reviews)\/?$/i` en la denylist. Verificado: el `sw.js` de producción incluye la denylist y el 302 a Google funciona.

### Vista previa Open Graph de /resena (2026-06-14)

Al compartir `/resena` por WhatsApp no salía ni imagen ni texto: un 302 a un dominio externo (Google) no da a los unfurlers ningún HTML con OG tags. Ahora `server/index.ts` distingue por User-Agent con `isCrawler` (`server/seo/constants.ts`, ya cubre WhatsApp/facebookexternalhit/Twitterbot/...): a los crawlers les sirve un HTML mínimo con OG/Twitter (`server/lib/reviewShareOg.ts` → `buildReviewShareHtml`), a los humanos el 302 de siempre. Como el crawler no conoce el teléfono, el idioma de la preview viaja en `?l=es|en`; el CRM lo construye desde el prefijo (`reviewShareLangForPhone`: +34 → es, resto → en) en `server/routes/admin-flywheel.ts` y `server/services/schedulerService.ts`, vía el parámetro `reviewUrl` de `renderThankYouWhatsApp`. La imagen es `og-image.webp` (1200x630). Nota: WhatsApp cachea previews de forma agresiva y sin purga pública; las URLs con `?l=` (las del CRM) son "nuevas" y sortean la caché negativa del `/resena` pelado ya compartido.

Desfase de "una versión por detrás": el SW que decide cómo servir *esta* petición es siempre el que ya estaba activo, así que un usuario con el SW viejo cacheado ve el 404 una vez y se autocorrige en la siguiente recarga (gracias a `skipWaiting` + `clientsClaim`, que minimizan la ventana a un refresh). No se puede forzar a un SW ya cacheado a comportarse distinto: su código está congelado hasta que el ciclo de vida lo reemplaza, y cualquier parche (denylist o redirect en el 404 del SPA) solo entra cuando se carga el bundle nuevo. Decisión 2026-06-14: **se mantiene el service worker** — quitarlo perdería cargas repetidas rápidas, PWA instalable y fallback offline a cambio de evitar un caso borde puntual ya resuelto y autocurable.
