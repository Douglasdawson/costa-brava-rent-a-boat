...
// OpenAI audio API response types (not fully typed in SDK)
export interface AudioMessagePart {
  audio?: { transcript?: string; data?: string };
  content?: string;
}

export interface AudioDeltaPart {
  audio?: { transcript?: string; data?: string };
}
...
export async function voiceChat(
  ...
  const message = response.choices[0]?.message as unknown as AudioMessagePart;
...
export async function textToSpeech(
  ...
  const audioData = (response.choices[0]?.message as unknown as AudioMessagePart)?.audio?.data ?? "";
