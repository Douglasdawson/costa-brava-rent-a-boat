...
- Extras: snorkel (7,50EUR), paddle surf (25EUR), nevera (5EUR), bebidas (2,50EUR/ud), parking (10EUR)
...
export async function publishNextDraft(): Promise<number> {
  // Publish only the oldest unpublished draft (1 per run = 1 per week)
  const [oldest] = await db
    .select({ id: schema.blogPosts.id, title: schema.blogPosts.title })
    .from(schema.blogPosts)
    .where(eq(schema.blogPosts.isPublished, false))
  ...
  await db
    ...
    .where(eq(schema.blogPosts.id, oldest.id));

  logger.info("BlogAutopilot published next draft", { id: oldest.id, title: oldest.title });
  return 1;
}
