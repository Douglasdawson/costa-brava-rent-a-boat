...
export interface IStorage {
  // Tenant methods (Multi-tenant SaaS)
...
export class DatabaseStorage implements IStorage {
  ...
  async updateTenant(id: string, data: UpdateTenant): Promise<Tenant | undefined> {
    const [tenant] = await db
      .update(tenants)
  ...
  async seedDefaultTenant(): Promise<Tenant> {
    ...
    if (existing) {
      // Still run migration in case some rows are missing tenantId
      await this.migrateDataToTenant(existing.id);
      return existing;
    }
    ...
    await this.migrateDataToTenant(tenant.id);
   ...
   * Uses raw SQL for simplicity and type safety across 22 tables.
   */
  private async migrateDataToTenant(tenantId: string): Promise<void> {
    const tableNames = [
