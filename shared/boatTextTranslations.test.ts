...
import { translateBoatText } from "./boatTextTranslations";
...
describe("translateBoatText", () => {
  ...
  it("translates the descriptive product names and leaves the model names alone", () => {
    expect(translateBoatText("Excursión Privada con Capitán", "en")).toBe(
      "Private Excursion with Skipper"
    );
    expect(translateBoatText("Astec 480", "en")).toBe("Astec 480");
    expect(translateBoatText("Trimarchi 57S", "de")).toBe("Trimarchi 57S");
    expect(translateBoatText("Pacific Craft 625", "fr")).toBe("Pacific Craft 625");
  });
